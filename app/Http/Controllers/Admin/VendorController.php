...
use App\Models\{User,Role,Category,VendorCommision,Deparment};
use Validator;
...
class VendorController extends Controller
{
    public function index()
    {
      ...
      $data['department'] = Deparment::where(['is_deleted'=>0,'active_status'=>1])->get();
      $data['cat'] = Category::where(['is_deleted'=>0,'active_status'=>1])->get();
      return view('admin.vendor',$data);
    }//end of method
    
    public function save(Request $request){
        // print_r(strtoupper(uniqid('INS-')));
        // print_r($this->random_strings(5));
        // die;
        $id = $request->id;
        if (!empty($id)) {
        ...
        if($validator->passes()) {
            ...
            $formdata['department'] = implode(',', $request->department);
            $formdata['emp_code'] = 'INS-'.$this->random_strings(5);
    ...
    public function show(Request $request){
        if ($request->ajax()) {
            $data = User::join('roles','roles.id','=','users.user_type')->where(['users.is_deleted'=>0,'users.user_type'=>3,'roles.is_deleted'=>0])->orderBy('id','DESC')->get(['roles.name as role','users.name','users.id','users.email','users.mobile','users.emp_code','users.image','users.commision','users.active_status','users.department']);
