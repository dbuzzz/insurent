...
class PriceController extends Controller
{
    ...
    public function save(Request $request){
        ...
        if($validator->passes()) {
            ...
            if (!empty($id)) {
                ...
                $row = Price::where('id',$id)->update($formdata);
                $msg = 'Data Updated';
            }else{
                $formdata['added_by'] = Auth::user()->id;
                $row = Price::insertGetId($formdata);
                $msg = 'Data Inserted';
    ...
    public function show(Request $request){
        if ($request->ajax()) {
            $data = Price::where(['is_deleted'=>0])->orderBy('id','DESC')->get();
    ...
    public function delete(Request $request){
        ...
        $row = Price::where('id',$request->id)->update($data);
