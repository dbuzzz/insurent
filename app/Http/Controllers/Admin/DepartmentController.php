...
class DepartmentController extends Controller
{
    ...
    public function save(Request $request){
        ...
        if($validator->passes()) {
            ...
            $formdata['name'] = $request->department;
            
            if (!empty($id)) {
                $formdata['updated_by'] = Auth::user()->id;
                $row = Deparment::where('id',$id)->update($formdata);
                $msg = 'Data Updated';
            }else{
                $formdata['added_by'] = Auth::user()->id;
                $row = Deparment::insertGetId($formdata);
                $msg = 'Data Inserted';
    ...
    public function show(Request $request){
        if ($request->ajax()) {
            $data = Deparment::where('is_deleted',0)->orderBy('id','DESC')->get();
    ...
    public function edit(request $request){
        $data = Deparment::where('id',$request->id)->first();
        // dd($data);
        if(empty($data)){
    ...
    public function delete(Request $request){
        ...
        $row = Deparment::where('id',$request->id)->update($data);
