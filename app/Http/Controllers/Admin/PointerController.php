...
class PointerController extends Controller
{
    public function index()
    {
      ...
      return view('admin.pointer',$data);
    }//end of method
    ...
    public function save(Request $request){
        ...
        if($validator->passes()) {
            ...
            if (!empty($id)) {
                ...
                $row = Ponter::where('id',$id)->update($formdata);
                $msg = 'Data Updated';
            }else{
                $formdata['added_by'] = Auth::user()->id;
                $row = Ponter::insertGetId($formdata);
                $msg = 'Data Inserted';
    ...
    public function show(Request $request){
        if ($request->ajax()) {
            $data = Ponter::where(['is_deleted'=>0])->orderBy('id','DESC')->get();
            return DataTables::of($data)
                    ...
                    ->addColumn('image', function($row){
                           ...
                           $btn = '<div style="background: '.$row->color.';height: 50px; width:50px"></div>';
                    ...
                    ->rawColumns(['descr','action','status','image'])
                    ->make(true);
        }
    }//end of method

    public function edit(request $request){
        $data = Ponter::where('id',$request->id)->first();
        // dd($data);
        if(empty($data)){
    ...
    public function delete(Request $request){
        ...
        $row = Ponter::where('id',$request->id)->update($data);
